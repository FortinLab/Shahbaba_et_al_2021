...
def load_data(rat_name):
    ...
    """
    datapath = ''
    trial_info = np.load('datapath/{}_trial_info.npy'.format(rat_name))
    spike_data = np.load('datapath/{}_spike_data_binned.npy'.format(rat_name))
    lfp_data = np.load('datapath/{}_lfp_data_sampled.npy'.format(rat_name))
